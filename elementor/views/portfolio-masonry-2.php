...
$thumb_size = 'neuzin-size' . rand( 6, 7 ) ;
		...
		<div class="<?php echo esc_attr( $col_class ). ' '; echo esc_html( $terms_of_item ); ?> rt-grid-item">
			<div class="rtin-item">
					...
					<a href="<?php the_permalink(); ?>">
						<?php
							if ( has_post_thumbnail() ){
								the_post_thumbnail( ('neuzin-size' . rand( 6, 7 ) ), ['class' => 'img-fluid mb-10 width-100'] );
							} else {
								if ( !empty( NeuzinTheme::neuzin_options('no_preview_image')['id'] ) ) {
									echo wp_get_attachment_image( NeuzinTheme::neuzin_options('no_preview_image')['id'], ('neuzin-size' . rand( 6, 7 ) ) );
								} else {
				...
				<div class="rtin-content">
					...
					<?php } ?>
					<div class="rtin-read"><a href="<?php the_permalink(); ?>"><?php esc_html_e( 'View Project', 'neuzin-core' );?></a></div>
					</div>
				</div>
			</div>
		</div>
