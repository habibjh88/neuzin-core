...
class NeuzinTheme_Open_Hour_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'Display the opening hours' , 'neuzin-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'open-hour', esc_html__( 'Neuzin : Opening Hours' , 'neuzin-core' ), $widget_ops );
		
		$this->weekdays = array(
			'monday'   => esc_html__( 'Monday', 'neuzin-core' ),
			'tuesday'  => esc_html__( 'Tuesday', 'neuzin-core' ),
			'wednesday'=> esc_html__( 'Wednesday', 'neuzin-core' ),
			'thursday' => esc_html__( 'Thursday', 'neuzin-core' ),
			'friday'   => esc_html__( 'Friday', 'neuzin-core' ),
			'saturday' => esc_html__( 'Saturday', 'neuzin-core' ),
			'sunday'   => esc_html__( 'Sunday', 'neuzin-core' ),
		);
	}
		...
		<ul class="opening-schedule">
			...
			<?php if( !empty( $from_label ) || !empty( $to_label ) ){ ?>
					<li><?php echo esc_html( $weekday ); ?><?php if( !empty( $from_label ) && !empty( $to_label ) ){ ?><span> <?php echo esc_html( $from_label ); ?> - <?php echo esc_html( $to_label ); ?> </span>
						<?php } else { ?><span class="os-close"><?php esc_html_e('Closed' , 'neuzin-core');?></span><?php } ?>
					</li>
				<?php } ?>
			<?php } ?>
		</ul>
	...
	public function form( $instance ) {		
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'neuzin-core' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<table>
			<tr>
				<th style="width: 30%"><?php esc_html_e( 'Days' , 'neuzin-core'); ?></th>				
				<th style="width: 30%"><?php esc_html_e( 'From' , 'neuzin-core'); ?></th>				
				<th style="width: 10%; text-align: center"></th>
				<th style="width: 30%"><?php esc_html_e( 'To' , 'neuzin-core'); ?></th>
			</tr>
		<?php foreach ( $this->weekdays as $key=>$weekday ) { ?>
			<?php			
				$from_label = $key.'_from';
